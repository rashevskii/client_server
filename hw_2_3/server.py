    ...
    while True:
        ...
        if command == 'help':
            ...
            for user in sorted(database.users_list()):
                print(f'User {user[0]}, last in: {user[1]}')
        elif command == 'connected':
            for user in sorted(database.active_users_list()):
                print(f'User {user[0]}, connected: {user[1]}:{user[2]}, time to set connection: {user[3]}')
        elif command == 'loghist':
            name = input('Enter username for view hystory. '
                         'For view all history, press Enter: ')
            for user in sorted(database.login_history(name)):
                print(f'User: {user[0]} time of in: {user[1]}. Enter with: {user[2]}:{user[3]}')
        else:
